...
class PredictiveParser:
    ...
    def parseString(self, input_string : str):
        ...
        '''
        self.input : List[str] =     input_string.split('.') + self.input 
        open('output/3-predictive-parsing.txt', 'w').truncate()
        while True :
            curr_stack : str = ''.join(self.stack)
            curr_input : str = ''.join(self.input)
            print(f"{curr_stack :<10} {curr_input}")
            with open('output/3-predictive-parsing.txt', 'a') as f :
                f.write(f"{curr_stack :<10} {curr_input}\n")
            try :
                if self.input == ['$'] and self.stack == ['$'] :
                    # Accepting state
                    return True 
                elif self.input[0] == self.stack[-1]:
                    # Cancelling condition
                    self.input.pop(0)
                    self.stack.pop()
                elif self.parsing_table[self.stack[-1]][self.input[0]] == 'e':
                    # Epsilon condition
                    self.stack.pop()
                else :
                    # Non-Terminal in stack expands
                    exp : str = self.parsing_table[self.stack[-1]][self.input[0]]
                    self.stack.pop()
...
with open('input/3-predictive-parsing.txt', 'r') as f:
    inputString = f.read()
